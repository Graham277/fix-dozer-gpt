...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            const res = await axios.get(`https://api.statbotics.io/v3/team_years?year=${dayjs().year()}&metric=${metric}&limit=15${region}`, config);
            ...
            if (error.response && error.response.status === 422) {
              ...
              interaction.editReply(`The ${year} season hasn't started yet!`)
            } else {
              console.error(error);
